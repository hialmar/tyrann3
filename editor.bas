10 TEXT:CLS:PAPER 0:INK3
15 POKE48035,0:POKE#26A,PEEK(#26A)AND254
20 S$="Characters Editor":GOSUB 1000:PRINT@8,4;S$
30 GOSUB 40000
100 REM EQUIPE
101 L1$=" * TYRANN 3 - TEAM *  "
102 L2$="CHARACTER   HOUSE     CAREER   LEV"
103 L3$="Money       ML RC ST DX IG MS HP  "
110 CLS:PRINT@8,1;CHR$(145)CHR$(128)+L1$+CHR$(144)
120 L=3:PRINT@3,L;CHR$(145)CHR$(128)+L2$+CHR$(144)
130 PRINT@3,L+1;CHR$(148)CHR$(128)+L3$+CHR$(144)
135 L=5:TE=0
140 FORI=1TO6
...
210  PRINT@1,L;CHR$(ENC);I;N$(I);
215  PRINT@17,L;M$(MP(I)):PRINT@27,L;C$(CP(I))
220  PRINT@37,L;NI(I)
230  PRINT@4,L+1;STR$(RI(I));" ca"
240  S1$=STR$(CC(I))+STR$(CT(I))+STR$(FO(I))+STR$(AG(I))
241  S2$=STR$(IN(I))+STR$(FM(I))+STR$(PV(I))
243  S$=S1$+S2$
250  PRINT@16,L+1;S$:L=L+3:TE=TE+RI(I)
260 NEXTI:PRINT@3,L-1;CHR$(148);CHR$(128)L3$CHR$(144)
290 EN=4
350 S$="CHOOSE HERO TO EDIT (1-6) ?":GOSUB 2000:PRINT@5,23;S$
355 S$="Save Changes: 0 or Q)uit":GOSUB 1000:PRINT@6,24;S$
360 GOSUB 3000:IF P=0THEN GOSUB30000:GOTO100
400 CLS:L=23:GOSUB 10000'menu edition
430 PRINT@6,3;"Fst Name:";N$(P)
440 PRINT@6,5;"Lst Name: ";M$(MP(P))
440 PRINT@6,7;"Career:";C$(CP(P))
450 PRINT@6,9;"Level:";NI(P)
450 PRINT@6,11;"Health Pts:";PV(P)
460 PRINT@6,13;"Purse:";RI(P);"s."
471 PRINT@6,15;"Melee Combat   :";CC(P)
472 PRINT@6,16;"Ranged Combat  :";CT(P)
473 PRINT@6,17;"Strength ......:";FO(P)
474 PRINT@6,18;"Dexterity .....:";AG(P)
475 PRINT@6,19;"Intelligence ..:";IN(P)
476 PRINT@6,20;"Mental Strength:";FM(P)
500 S$="Arrows or Q)uit":GOSUB1000:PRINT@4,22;S$
510 S$="M)odif":GOSUB1000:MO$=S$
520 S$="<- +>":GOSUB1000:FLECHE$=S$
600 REM EDITION
610 X=26:LI=3:UP=0:L=1
620 REPEAT
630  PRINT@4,LI;"=>":IFLI<8THENPRINT@X,LI;MO$ELSEPRINT@X,LI;FL$
640  GETA$
...
730 IFA$="Q"THEN100
790 PRINT@4,LI;"=>":IFLI<4THENPRINT@X,LI;MO$ELSEPRINT@X,LI;FL$
...
2020 RETURN
3000 GETA$:IF A$="Q" OR A$="q" THEN END
3010 P=VAL(A$):IFP>6THENPING:GOTO 3000
3010 RETURN
3500 S$="< SPACE > ":GOSUB2000
3501 PRINT@13,L;S$:GETA$:IF A$<>" "THEN3500
3505 RETURN
...
6230 GETM$:M=VAL(M$)
6235 IFM<1ORM>9THENZAP:GOTO6230
6240 MP(P)=M:PING
6250 RETURN
6300 REM CARRIERE
6310 FORI=1TO6
...
6330 GETM$:C=VAL(M$)
6335 IFC<1ORC>6THENZAP:GOTO6330
6340 CP(P)=C:PING
6350 RETURN
6400 REM LEVEL
...
30010 S$="SURE YOU WANT TO SAVE CHANGES ?":GOSUB2000
30011 PRINT@3,23;S$
30012 S$="             (Y/N)             ":GOSUB2000
30015 PRINT@3,24;S$
30020 A$=KEY$:IFA$=""THEN30020
30030 IFA$<>"Y"THEN 100
...
40030 PRINT@5,24;"INSERT DISK IN DRIVE"
40040 PRINT@5,25;"  THEN PRESS SPACE"
40050 IFKEY$<>" "THENGOTO40040
48000 GOSUB49600
48005 LOAD"TEAM.BIN"
48010 O1=#A000
...
48350 RETURN
49000 TEXT:CLS
49005 PRINT@8,2;CHR$(145);CHR$(135);"+ TEAM PREP + ";CHR$(144)
...
49400 RETURN
49600 CLS:PRINT@7,8;"<<< Loading <<< Wait <<<"
49610 S$=CHR$(148)+" "+CHR$(144):CU=1:GOTO49650
49620 PRINT@7,8;">>> Saving >>> Wait  >>>"
49630 S$=CHR$(145)+" "+CHR$(144):CU=1
49650 CU=CU+2:PRINT@CU,9;S$:REM FR = FRE("")
49660 RETURN
50000 DATA Knight,Mercenary,Ranger,Sorcerer,Maestre,Septon
50020 DATA None,MARTELL,BARATHEON,TYRELL,GREYJOY,ARRYN,LANNISTER,TULLY,STARK