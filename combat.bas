10 REM {++ ORIC 2014 + TYRANN 3 CBV1 + MAXIMUS & HIALMAR +++}
17 PLOT 0,6,"< ESPACE >":PING
18 IF KEY$<> " " THEN 18
19 A=DEEK(#308):R= RND(-A)
20 GRAB : HIMEM 40959 : GOSUB 48000: GOSUB 52100 : POKE 48035,0
...
300 REM 
310 T=INT((42-LEN(S$))/2):PRINT @T,L;S$
320 RETURN
...
600 PING:PRINT @2,19;CHR$(148)" IMPOSSIBLE "CHR$(145)
610 WAITTI*5:GOSUB 8300
620 RETURN
...
3000 REM |===/ COMBATS \===|
3010 T=13:IF NC=2 THEN T=8 ELSE IF NC=3 THEN T=0'niveau
3030 NE=FNA(4)+1:EV=NE
3050 FOR I=1 TO NE
3060  SS=FNA(NM-T):MO(I)=SS
3080  C1AGI(I)=CM(SS,1)+FNA(VIL)
3090  C2PV(I)= CM(SS,2)+FNA(VIL):DC=DC+C2(I) 
...
3200 REM INIT
3210 FU=0:ZO=0:HV=6:DR=0:AMI=0:FF=0
...
4205 FR = FRE("")
4210 IF DRAG>0 THEN GOSUB 33850
4220 FOR P=1TO6+NE 
4225 BF=BF(AO(P))
4230  ACT$=" attaque ":GOSUB 700
4240  PRINT@2,7;CHR$(145)"------------- COMBAT ------------- "CHR$(144)
4250  IF ESP(P)=0 THEN GOTO 6000'action ennemis  
4300   REM HEROS
...
6020 IF C6OK(AO(P))=4 THEN GOSUB 36000:GOTO 6200
...
7020 REM PRINT @5,12;"1.Recommencer"
7030 REM PRINT @5,14;"2.Battre en retraite"
7050 GOSUB 8300
7060 REM GETA$:IF A$<"1" OR A$>"2" THEN 7060
7070 GOSUB 49000 : FR=FRE("") : RELEASE : LOAD "CREATION"
7080 REM CLS:PRINT@15,5;"Pleutre !":ZAP
7090 REM PRINT@5,8;"Memoire restante:";:PRINT FRE(""):PING:END 

8000 REM |SOUS PROGS COMBAT 

8100 REM AFFICHE ENNEMIS
8110 FORI=1TO6:PRINT@1,I;"                               ":NEXT
8115 PRINT@3,6;"DC:";DC
8116 ZO=1
8120 FORI=1TONE
...
8910 REM FUITE
8915 FF=1:PRINT@30,11;CHR$(131)"FUIR O/N"
8920 GETA$
8925 IF A$<>"O" AND A$<>"N" THEN 8920
8930 IF A$="N" THEN 8980
8935 IF FNA(50)+NF>AG(FNA(6)) THEN NF=0:GOTO 8970
8940 CLS:PRINT@13,8;CHR$(145)" VOUS DETALEZ !!"CHR$(144)
8945 PRINT@13,10;"RETOUR LABYRINTHE":PRINT:NF=NF+10
8950 FORJ=1TO5:ZAP:WAIT5:NEXT:SHOOT:WAIT5
8960 GOSUB 49000 : FR=FRE("") : RELEASE : LOAD("LABY")
8970 PRINT@30,11;CHR$(131)"  ECHEC  ":SHOOT:WAITTI*4
8980 PRINT@30,11;CHR$(131)"         "
8990 RETURN
...
9060 IF A$="3" THEN IF BT(P)>0 THEN BF(P)= IA(BT(P)-41):AU(P)=BT(P) ELSE 9030
9070 IF AU(P)=18 AND FU=1 THEN PRINT@25,16;" FILET ACTIF ":WAIT 5*TI:GOSUB 700:GOSUB 8500:GOTO 8700
9080 BF(P)=BF(P)+FC(P):GOSUB 9600
9090 WAIT 5*TI
9100 RETURN
...
9270 IF OC(P)>19 AND OC(P)<23 THEN GOSUB 9500:GOTO 9350 
9280 IF OC(P)=23 THEN GOTO 9350
9290 IF OC(P)>30 AND OC(P)<36 THEN PING:GOTO 9350
9300 IF OC(P)=37 OR OC(P)=38 THEN GOTO 9350
...
9710 REM UTIL OBJET
9720 IF (OC(AO(P))>22 AND OC(AO(P))<31) OR OC(AO(P))=37 OR OC(AO(P))=38 THEN 9770
9730 IF OC(AO(P))>19 AND OC(AO(P))<22 THEN PRINT @3,10;CHR$(130);N$(AO(P))" Soigne "N$(TG((AO(P)))):GOSUB 32130
9740 IF OC(AO(P))=22 THEN PRINT @3,10;N$(AO(P))" utilise Essence Vitale ":PRINT @5,13;N$(TG((AO(P))));" revient a la vie":GOSUB 9950
9750 IF OC(AO(P))>30 AND OC(AO(P))<36 THEN GOSUB 10000:PRINT @3,10;M$
9760 WAIT 5*TI:GOSUB 8300:GOTO 9790
9770 IF OC(AO(P))=23 THEN PRINT @3,10;N$(AO(P))" utilise la vision Zoman ":ZO=1:GOTO 9790
9780 IF OC(AO(P))=37 OR OC(AO(P))=38 THEN PRINT @3,10;N$(AO(P))" Fait peter ";IT$(OC(AO(P))):GOSUB 10100:GOSUB 9850:GOTO 9790
...
9870 C6OK(REVEIL)=1:EV=EV+1:GOSUB 8100
9880 RETURN
...
10000 REM FOOD
10010 IF OC(AO(P))=31 THEN M$=" boit de l'eau ":P1=4:P2=2:GOTO 10060
10020 IF OC(AO(P))=32 THEN M$=" mange du pain ":P1=5:P2=3:GOTO 10060
10030 IF OC(AO(P))=33 THEN M$=" sirote la cervoise ":P1=8:P2=3:GOTO 10060
10040 IF OC(AO(P))=34 THEN M$=" engloutit le poisson ":P1=8:P2=4:GOTO 10060
10050 IF OC(AO(P))=35 THEN M$=" devore le sanglier ":P1=10:P2=6
10060 SS=FNA(P1)+P2:M$=N$(AO(P))+M$
10070 ET(AO(P))=ET(AO(P))+SS:IF ET(AO(P))>PV(AO(P)) THEN ET(AO(P))=PV(AO(P))
10090 RETURN

10100 REM ++ GREGEOIS & SORTS COLLECTIFS
10110 DEG=6:IF OC(AO(P))=37 THEN DEG=3
10120 FORJ=1TO5:SHOOT:WAIT15:PAPER J:NEXTJ:EXPLODE:PAPER0:GOSUB 700
10130 IF EV=0 THEN 10290
10140 LG=0:REVEIL=0
10150 FORI=1TO NE
10160 IF C6OK(I)<1 THEN 10240
10165 IF C6OK(I)=5 THEN REVEIL=I:PING
10170 LG=LG+1
10175 SS=FNA(DEG)+DEG:IF C6OK>4 THEN SS=SS*3
10180  S$=MM$(MO(I))+" perd "+STR$(SS)+" PV"
10190  C2PV(I)=C2(I)-SS:IF C2(I)>0 THEN 10220
10200  S$=S$+CHR$(129)+" et meurt":IF C6OK(I)=6 THEN FU=0 
10210  EV=EV-1:C6OK(I)=0:MT(AO(P))=MT(AO(P))+1
10220 PRINT @3,9+LG;S$:WAIT TI*5
10240 NEXT I
10250 IF EV<=0 THEN 10290
10260 GOSUB 8100:WAIT TI*5
10290 RETURN

10499 REM +++ Tri des persos = Action Order
...
11040  IF AU(AO(P))=15 THEN ACT$=" decoche un carreau sur ":DFF=15:GOTO 11100 
11050  IF AU(AO(P))=16 THEN ACT$=" decoche une fleche sur ":DFF= 5:GOTO 11100 
...
11205 IF C6OK(TG(AO(P)))>4 THEN RT=1
11210 IF RT=0 THEN PING:PRINT@15,14;" et loupe ! ":GOTO 11260
11215 SS=VIL+FNA(5):IF EF(AO(P))=0 THEN SS=SS+BF(AO(P))
11216 IF EF(AO(P))>0 THEN SS=SS+((1+FNA(2))*(BF(AO(P))))
11217 IF C6OK(TG(AO(P)))>4 THEN SS=SS+C2PV(TG(AO(P))):GOTO11230
11220 SS=SS-MALUS:IF SS<0THENSS=1
11230 C2PV(TG(AO(P)))=C2PV(TG(AO(P)))-SS
11240 PRINT @12,14;"lui inflige "SS" pv  "
11250 IF C2PV(TG(AO(P))) <= 0 THEN MT(AO(P))=MT(AO(P))+1:S$=MM$(MO(TG((AO(P))))):GOTO 40000
11260 WAIT 15*TI
11290 RETURN
...
20070 FU=1:GOTO 20150:REM IF FU=0 THEN 20150 désactivé pour le moment
20080 I=0
20090 REPEAT
20100 I=I+1:B$=MM$(MO(I))
20110 UNTIL C6OK(I)=6 OR I=NE 
20115 IF C6OK(I)=6 THEN MO(1)=MO(I) ELSE 20150
20120 PRINT@4,13; "Vous avez un prisonnier: ":PRINT @6,15;"> "B$
20150 FOR P=1TO6
...
20200 S$="Voila une prime de"+STR$(NE*50)+" ca":L=14:GOSUB 300:WAITTI*12
20201 RI(P)=RI(P)+NE*50:XP(P)=XP(P)+XP+(MT(P)*20)+FNA(10*VIL)
20210 L=16:GOSUB 400:GOSUB 22000
20220 IF XP(P)>100+(VIL*50) AND NI(P)<21 AND OK(P)<>4 THEN GOSUB 21000 
20250 NEXT P
20260 L=21:GOSUB 400
20280 CLS:PRINT@12,6;"* Patientez *":PING
20290 CA=0
20300 GOSUB 49000 : FR=FRE("") : RELEASE : LOAD("LABY")

21000 REM PROMOTION NEW
21010 NI(P)=NI(P)+1:XP(P)=0:FORJ=1TO5:PING:WAIT2*J:NEXT
21020 S$=CHR$(145)+C$(P)+" "+N$(P)+" "+CHR$(144)+CHR$(132):L=11:GOSUB 300
...
21060 PRINT@6,15;CHR$(145)CHR$(135);S$;CHR$(144)CHR$(132)
21070 S$=STR$(CC(P))+" "+STR$(CT(P))+" "+STR$(FO(P))+" "+STR$(AG(P))+" "+STR$(IN(P))+" "+STR$(FM(P))
21080 PLOT 7,16,S$
21100 PRINT @5,18;"Augmenter quelle carac (1-6) ?"
21110 GOSUB 500:PROMO=5+FNA(4)
21120 IF A=1 THEN CC(P)=CC(P)+PRO:GOTO 21180
21130 IF A=2 THEN CT(P)=CT(P)+PRO:GOTO 21180 
21140 IF A=3 THEN FO(P)=FO(P)+PRO:GOTO 21180 
21150 IF A=4 THEN AG(P)=AG(P)+PRO:GOTO 21180 
21160 IF A=5 THEN IN(P)=IN(P)+PRO:GOTO 21180 
21170 IF A=6 THEN FM(P)=FM(P)+PRO 
21180 S$=STR$(CC(P))+" "+STR$(CT(P))+" "+STR$(FO(P))+" "+STR$(AG(P))+" "+STR$(IN(P))+" "+STR$(FM(P))
21200 PLOT 7,17,S$
21210 PV(P)=PV(P)+FNA(4)+3
21250 L=21:GOSUB 400:GOSUB 22000
21300 RETURN

22000 FOR J=1TO11:PRINT @4,10+J;"                                 ":NEXTJ
22010 RETURN
...
26322  IF SP(AO(P))=8 AND SD=0 THEN S$=" Et la selle ? "L=12:GOSUB 300:GOTO27000
...
31000 REM SORCIER
31100 REM 1,1 
31120 TST=5:DFF=10:GOSUB 15000:IF RT=0 THEN 35000
31130 C6OK(TG(AO(P)))=5:S$="pique un roupillon...ZZZzzz":GOSUB 35000
31190 RETURN

31200 REM 1,2  FEU 
31210 TST=5:DFF=15:GOSUB 15000:IF RT=0 THEN 35000
...
33110 TST=5:DFF=0:GOSUB 15000:IF RT=0 THEN EF(TG((AO(P))))=0:GOTO 35000
...
33200 REM 3.2 FORCE
33210 TST=5:DFF=10:GOSUB 15000:IF RT=0 THEN 35000
33215 IF OK(TG(AO(P)))=4 THEN 33290
33220 S$=" sa force grandit ":PRINT @12,14;S$:WAIT TI*10
33230 FC(TG(AO(P)))=FC(TG(AO(P)))+FNA(2)+1
33290 RETURN

33300 REM 3.3 CHARME
33310 TST=5:DFF=15:GOSUB 15000:IF RT=0 THEN 35000
33320 AMI=TG(AO(P)):C6OK(AMI)=4:EV=EV-1
33330 S$="Et voila un nouvel ami ":PRINT @4,14;S$
33390 RETURN
...
33520 PRINT @10,12;"Le souffle du Nord  !!  ":WAIT TI*10
33530 'test de Force ou Endurance  pour tous les ennemis, si echec > mort de froid
33590 RETURN

33600 REM 3.6 ILLUSION
33610 TST=4:DFF=25:GOSUB 15000:IF RT=0 THEN 35000
...
33720 TST=5:DFF=20:GOSUB 15000:IF RT=0 THEN 35000
...
33810 DR=DR+1:IF DR=3 THEN DR=2
33820 S$="Un Dragon se joint a vous":GOSUB 35000:GOSUB 700
33840 RETURN

33850 FOR D=1TODR
33855 GOSUB 700:S$=" VOTRE DRAGON MAGIQUE NUM "+STR$(D):T=INT((41-LEN(S$))/2)
33860 PRINT @T,7;S$:S$=CHR$(129)+"! le Dragon se dechaine ! "
...
36040 TE=FNA(NE):IF (C6OK(TE)>3 OR C6OK(TE)=0) AND EV>1 THEN 36040
36060 SS=FNA(5)+2+C4(AMI):PRINT @4,12;MM$(MO(TE));" perd ";SS;" pv";:WAIT TI*15
36070 C2(TE)=C2(TE)-SS:IF C2(TE)<0 THEN C2(TE)=0:C6(TE)=0:GOTO 40050
36100 RETURN

36200 REM TEST Opposé 
36210 FORJ=1TONE
...
49350 FOR V=1TO8:FORM=1TO5:O1=O1+1:POKEO1,TC(V,M):NEXT M,V:PRINT "COMBATS";CO;TC(VILLE,1);O1;(O1-#A000)
49360 O1=O1+1:POKEO1,DE
49370 O1=O1+1:POKEO1,TL
49390 PING:SAVEU "TEAM.BIN",A#A000,EO1
49400 RETURN

49600 REM CLS:PRINT@6,8;".. Chargement * Patientez .."
49610 REM S$=CHR$(148)+" "+CHR$(144):CU=1:GOTO49650
...
50070 FORI=1TO8:READ M$(I):NEXTI:PRINT ".";
50080 READ M$(9)
50100 DIM IMPACT(19)
50110 REM GOSUB 52100
50115 REM RECALL  CM,"T-MONST",S
...
50310 DATA Chevalier,Paladin,Ranger,Sorcier,Mestre,Septon
50320 DATA AUCUNE,MARTELL,BARATHEON,TYRELL
50330 DATA GREYJOY,TULLY,LANNISTER,ARRYN,STARK

50400 DATA 7,6,5,5,4,4,3,3,2,2,1,1

50410 DATA 60,6, 70,4, 50,3, 85,3, 90,2, 50,1
...
51020 DATA EPEE-FEU, FORCE, CHARME, VISION, GLACE, ILLUSION, VENT, DRAGONS

52000 DATA "j'abaisse votre garde !","** Boules de feu **","Pluie de lames !","Je detruis vos armes !","Je soigne mes amis"

52090 REM Lecture TITEMS.BIN
52100 CLS:PRINT @ 8,12;CHR$(145);CHR$(135);"++ VEUILLEZ PATIENTER ++ ";CHR$(144):
52110 DIM ITEM$(55), CM(23,5)
52120 LOAD "TITEMS.BIN"
53110 O1=#A000
53120 LI=PEEK(O1)
53125 REM PRINT "LG IT:";LI
53130 FOR I=1 TO LI
...
53190   NEXT
53200   ITEM$(I)=S$
53205   REM PRINT "ITEM ";I;" = ";ITEM$(I)
53210 NEXT

53300 REM Lecture TMONST.BIN
53310 LOAD "TMONST.BIN"
53335 O1=#A000
53345 LI=PEEK(O1)
53346 O1=O1+1:LG=PEEK(O1)
53350 FOR I=1TOLI
53355  FOR J=1TO LG
53360   O1=O1+1:CM(I,J)=PEEK(O1)
53365  NEXTJ
53370 NEXTI
